...
export const api = {
   ...
   *
   * `selfieDataUrl` is the profile photo stored as `avatarUrl`.
   */
  ...
  /** Creator: reclaim unspent LMSR subsidy after resolution. */
  prepareReclaim: (marketId: string) =>
    request<{ transaction: string; amount: string }>(
      `/api/markets/${marketId}/reclaim`,
      { method: "POST" },
    ),

  /**
   * Submit a resolution photo: describe → validate → policy, then attempt
   * on-chain settle. 409 once a record exists unless the caller is the owner.
   */
  resolveMarket: (
    marketId: string,
