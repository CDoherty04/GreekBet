...
3. People in the description may be labelled with a group member's name (when the describer matched them to a profile photo) or with "Person 1", "Person 2", and so on. When the question names a person:
   - if the description already labels someone with that name (or a clear match), you may treat them as that person;
   - if only generic "Person N" labels appear, you may treat a described person as the named one only if exactly one described person could plausibly fit — say in "reasoning" that identity was inferred without a profile match, and lower "confidence";
   - if several described people could be the named person and it matters to the answer, add a red flag.
