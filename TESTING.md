...
1. **Sign up** at `/onboarding` — any name and phone, then take a **profile
   photo**. A Solana wallet is provisioned automatically.
...
7. **Resolve.** Upload a photo of the event. AI suggests yes/no from the photo
   (and can match people to group profile photos); the owner confirms on chain.
